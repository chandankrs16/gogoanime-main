...
const KEYS_URL = "https://raw.githubusercontent.com/justfoolingaround/animdl-provider-benchmarks/master/api/gogoanime.json"
...
export const scrapeMP4 = async({ id }) => {
    ...
    try {
        ...
        // replace sources with sources_bk
        res.source.forEach(source => sources_bk.push(source))
...
export const animeinfo = async() => {
    try {
        const keys = await fetch_keys();
        const iv = keys.iv;
        const key = keys.key;
        const second_key = keys.second_key;
        ...
        return {
            ...
            isLive:true,
            isUpdateLatest:true,
            isUpdateLatest1:true,
            isUpdateLatest2:true,
            isAnimeInfoAdsShow:true,
            isGenresAds:true,
            isPlayerAds:false
        }
